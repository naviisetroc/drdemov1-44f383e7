...
export default function PacienteChat() {
  ...
  // Show font-size tip every time the page loads
  useEffect(() => {
    const t = setTimeout(() => {
      toast.custom(() => (
        <div className="w-full max-w-sm rounded-2xl bg-gradient-to-r from-primary to-accent p-5 shadow-xl text-white flex items-start gap-4">
          <div className="h-14 w-14 rounded-xl bg-white/25 flex items-center justify-center shrink-0 mt-0.5">
            <span className="font-bold leading-none tracking-tight text-white" style={{ fontFamily: "system-ui, sans-serif" }}>
              <span className="text-3xl">A</span><span className="text-lg">a</span>
            </span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-bold text-lg leading-tight text-white">¿Necesitas texto más grande?</p>
            <p className="text-base text-white/90 mt-1 leading-snug">
              Pulsa el botón <strong className="text-lg text-white" style={{ fontFamily: "system-ui, sans-serif" }}>Aa</strong> en la esquina superior derecha para aumentar el tamaño de la fuente.
            </p>
          </div>
        </div>
      ), { duration: 10000 });
